...
Type4-2 규칙:
  - KOSPI: 시가총액 상위 30, S&P500: 시가총액 상위 100만 매수 후보 (type4 동일)
  - 매수는 `-→+` 전환 + 시총 순위 조건 만족 시에만 실행
  - 매수 시 가용 현금으로 살 수 있는 최대 주수(1주 단위) 매수
  - 초기 자본은 첫 매수 시 SHARES_PER_TRADE * 첫매수가로 자동 설정
  - 매도 시 보유 전량 매도, 매도 대금은 현금으로 보유
  - 손실 매도 시 다음 매수 주수가 줄어들고, 이익 매도 시 늘어남

type4 와의 차이:
  - type4: 매번 고정 SHARES_PER_TRADE (10주) 매수 (현금 무관)
  - type4-2: 가용 현금 안에서 최대 주수 매수 (손익에 따라 주수 변동)
"""
...
def default_output_csv() -> str:
    return "backtest_type4_2.csv"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Type4-2 백테스트: KOSPI 상위 30 / S&P500 상위 100 시가총액 조건을 만족하는 종목만 "
            "MA10M 기준 -→+ 전환에서 가용 현금으로 최대 주수 매수하고, +→- 전환에서 전량 매도합니다. "
            "초기 자본은 첫 매수 시 자동 설정됩니다."
        ),
        epilog=(
            "예시:\n"
            "  uv run python backtest_type4_2.py\n"
            "  uv run python backtest_type4_2.py --from 2020-01-01 --to 2026-04-12\n"
            "  uv run python backtest_type4_2.py --output_csv data/backtest_type4_2.csv"
        ),
        formatter_class=argparse.RawTextHelpFormatter,
    )
    parser.add_argument("--from", dest="from_date", default=DEFAULT_FROM_DATE, help="백테스트 시작일 (YYYY-MM-DD, 기본값: 2020-01-01)")
    parser.add_argument("--to", dest="to_date", default=default_to_date(), help="백테스트 종료일이자 평가 기준일 (YYYY-MM-DD, 기본값: 오늘)")
    parser.add_argument("--output_csv", dest="output_csv", default=default_output_csv(), help="결과 CSV 저장 경로 (기본값: backtest_type4_2.csv)")
    return parser.parse_args()


def simulate_type4_2(
    df: pd.DataFrame,
    ...
    if period_df.empty:
        return {**empty_result(window, 0.0, "-", "기간 데이터 없음"), "cash": 0.0, "initial_capital": 0.0}
    ...
    if period_df.empty or len(signal_df) < 2:
        return {
            **empty_result(window, float(df["Close"].iloc[-1]), "-", "신호 없음"),
            "cash": 0.0,
            "initial_capital": 0.0,
        }
...
def run_group_backtest(
    ...
    items: list[dict],
    window: BacktestWindow,
    rank_contexts: dict[str, RankContext],
) -> pd.DataFrame:
    rows: list[dict] = []
    ...
    for item in items:
        ...
        if df is None:
            continue
        result = simulate_type4_2(df, window, group_name, ticker, rank_context)
        rank_str = (
...
def print_group_result(group_name: str, window: BacktestWindow, df: pd.DataFrame) -> None:
    bar = "=" * 160
    print(f"\n{bar}")
    print(f"[{window.title}] {group_name} Type4-2 백테스트 (현금 추적)")
    print(bar)
    if df.empty:
        print("  결과 없음")
        return
...
def main() -> None:
    ...
    print("Type4-2 백테스트를 시작합니다...")
    print(f"기준일: {datetime.now().strftime('%Y-%m-%d')}")
    print(f"기간: {window.title}")
    print(f"규칙: KOSPI 상위 {KOSPI_TOP_N} / S&P500 상위 {SP500_TOP_N} 조건 + 첫 매수 = {SHARES_PER_TRADE}주로 초기자본 설정, 이후 가용 현금으로 최대 주수 매수 / +→- 전량 매도")
